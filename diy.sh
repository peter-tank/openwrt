...
'https://github.com/peter-tank/luci-app-clash.git^luci-app-clash,master^0fe7ae2c0627a44da80dabd99d610b1cf12b05bf'
'https://github.com/peter-tank/openwrt-v2ray.git^v2ray,master^52b09e3e53326b1b191a69fb143c1e0eb2d2fd1c'
'https://github.com/trojan-gfw/openwrt-trojan.git^openwrt-trojan,master^ffaed6a488bcd13bf4e23d58885db9c1205caf09'
'https://github.com/peter-tank/luci-app-ssr-plus.git^luci-app-ssr-plus,dev^136b8d87bbcd804dc4e4cb403e50e4bd0e8e6291'
'https://github.com/peter-tank/luci-app-frpc.git^luci-app-frpc,master^98e0451fae511196f817659ce69659dd4450c310'
'https://github.com/peter-tank/openwrt-frp.git^frp,master^cbc949440b98eee80db8685329f2dd86ab9e3e7e'
'https://github.com/peter-tank/luci-app-dnscrypt-proxy2.git^luci-app-dnscrypt-proxy2,master^8194837d07cb24b2f1f1a1a68339ce9e86632116'
...
'https://github.com/pexcn/openwrt-chinadns-ng.git^chinadns-ng,master^3ea5a6793014f1b409cc39508f9ff321204bf865'
'https://github.com/pexcn/openwrt-dns2tcp.git^dns2tcp,master^fc4826c9ca6f5511c90359180e05978ef9f79f08'
...
echo "# remove frpc frps init script & force to 0.32.0"
...
echo "# ipt2socks bleeding(1.1.2-1)"
file="$(pkg_path ipt2socks)/Makefile"
#sed -i -e '/[\t ]*DEPENDS:=/d' $file
#sed -i -e '/^PKG_BUILD_DIR:=/ {' -e 'n; iPKG_BUILD_DEPENDS:=libuv' -e '}' $file
sed -i -e 's/^\(.*\)b9bd7d888f2f95a6d7306030c8a5e252732dc451$/\1cfbc2189356aba7fcafb0bc961a95419f313d8a7/' $file
grep -Hn -A2 PKG_BUILD_DIR:= $file
grep -Hn -C3 TITLE:= $file
grep ^MAKE_FLAGS $file

echo "# bump trojan to testing(1.15.1-2) & port reuse enabled & force openssl1.1, boost linking static"
file="$(pkg_path trojan)/Makefile"
#sed -i -e 's/^PKG_VERSION:=.*$/PKG_VERSION:=1.15.1/' $file
sed -i -e 's/^PKG_RELEASE:=.*$/PKG_RELEASE:=2/' $file
sed -i -e 's/^\(.*\)0bad2988c60200c15786baf6b9ea42d66ba2109c$/\1edd91198adab442f90c1c513cd6bb35a16b03e94/' $file
sed -i -e 's/^PKG_SOURCE_URL:=.*$/PKG_SOURCE_URL:=https:\/\/github.com\/peter-tank\/trojan.git/' $file
...
echo "# force openssl1.1 version 1.1.1-g"
...
echo "# patch v2ray rm default V2RAY_COMPRESS_UPX y on SMALL_FLASH, bump to bleeding(4.23.1)"
