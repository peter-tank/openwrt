...
'https://github.com/trojan-gfw/openwrt-trojan.git^openwrt-trojan,master^0c0dc27b27d33a988826f9c38e7eac36d059b17a'
'https://github.com/peter-tank/luci-app-ssr-plus.git^luci-app-ssr-plus,master^0428f19406105554242755ec2f95264694717ec0'
'https://github.com/peter-tank/luci-app-frpc.git^luci-app-frpc,master^083929fa2e3932ceb964f435b7031edf3fd02e1a'
'https://github.com/peter-tank/openwrt-frp.git^frp,master^007e9de0f5e038a62cc9878170588b5eddc4dec2'
'https://github.com/peter-tank/luci-app-dnscrypt-proxy2.git^luci-app-dnscrypt-proxy2,master^2d9fcb4f2f7d3f98d8a51ea2db3ff241d9edc1d6'
'https://github.com/peter-tank/openwrt-autorepeater.git^luci-app-autorepeater,master^f5d07be7a8f607123a77c57090179f6d677ea8bf'
'https://github.com/pexcn/openwrt-chinadns-ng.git^chinadns-ng,master^c120f78a5fe336ba6d0e04c22f4b47f63ba5d283'
'https://github.com/pexcn/openwrt-dns2tcp.git^dns2tcp,master^923ce09111442072238e9e2ad8772b8366517a75'
...
echo "# ipt2socks bleeding(1.01-4) & fix static linking"
file="$(pkg_path ipt2socks)/Makefile"
sed -i -e '/[\t ]*DEPENDS:=/d' $file
sed -i -e '/^PKG_BUILD_DIR:=/ {' -e 'n; iPKG_BUILD_DEPENDS:=libuv' -e '}' $file
sed -i -e 's/^\(.*\)b9bd7d888f2f95a6d7306030c8a5e252732dc451$/\12ca99f4392cdbbbb5d303ff117dacc89ee572c38/' $file
grep -Hn -A2 PKG_BUILD_DIR:= $file
grep -Hn -C3 TITLE:= $file
grep ^MAKE_FLAGS $file

echo "# bump trojan to bleeding(1.15.1) & port reuse enabled & force openssl1.1.1e, boost linking static"
...
#sed -i -e 's/^\(.*\)7cfe059c664e06bc7663620024578d85d9e6008c$/\10bad2988c60200c15786baf6b9ea42d66ba2109c/' $file
