...
echo "patch golang patch to v1.16"
find -P . -path */golang/golang-version.mk
file="feeds/packages/lang/golang/golang-version.mk"
ls -la $file
sed -i -e 's/^GO_VERSION_MAJOR_MINOR:=.*$/GO_VERSION_MAJOR_MINOR:=1.16/' $file
sed -i -e 's/^GO_VERSION_PATCH:=.*$/GO_VERSION_PATCH:=/' $file
grep -Hn -A10 "^GO_VERSION_MAJOR_MINOR" $file
file="$(pkg_path golang)/Makefile"
sed -i -e 's/^PKG_HASH:=.*$/PKG_HASH:=7688063d55656105898f323d90a79a39c378d86fe89ae192eb3b7fc46347c95a/' $file
grep -Hn -C3 "^PKG_HASH" $file
   ;;
   ath79 )
